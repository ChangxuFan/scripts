...
awk -F "\t" -v TESUBF=${TESUBF} 'BEGIN{OFS="\t"} $4 == TESUBF && $4 == $(NF-5) {if ($(NF-6) == "+") print $(NF-5), $(NF-2)+$5, $(NF-2)+$6, $1, $2, $3,$7, $(NF-2), $(NF-1); if ($(NF-6) == "-") print $(NF-5), $(NF-1)-$6, $(NF-1)-$5, $1, $2, $3, $7, $NF, $(NF-1);}'  > ${OUTDIR}/${OUTTAG}-${TESUBF}.conss_color
